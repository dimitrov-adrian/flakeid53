...
describe('Initialization and setup', function () {
    ...
    it('Check workerId out of range', () => {
        expect(() => {
            createFlakeID53({ epoch: +new Date('2020-03-10T12:34:56.123Z'), workerId: 'a' });
        }).toThrow();
    });

    it('Check workerId in range', () => {
        expect(() => {
            createFlakeID53({ epoch: +new Date('2020-03-10T12:34:56.123Z'), workerId: 6 });
        }).not.toThrow();
    });

    it('Int is less than max safe int', async () => {
        const flakeId = createFlakeID53({ epoch: +new Date('2020-03-10T12:34:56.123Z'), workerId: 1 });
    ...
    it('Year space', async () => {
        ...
        for (let y = currentYear - 28; y < currentYear; y++) {
            const flakeid = createFlakeID53({
                epoch: +new Date(`${y}-${new Date().getUTCMonth()}-${new Date().getUTCDate()}`),
            });
...
describe('Parsing', function () {
    it('First', async () => {
        const flakeId = createFlakeID53({ epoch: +new Date('2020-03-10T12:34:56.123Z'), workerId: 0 });
    ...
    it('Some', async () => {
        const flakeId = createFlakeID53({ epoch: +new Date('2020-03-10T12:34:56.123Z'), workerId: 8 });
...
describe('Generation', function () {
    it('Sequence', async () => {
        ...
        const generatedList = [];
        for (let i = 0; i < 100; i++, generatedList.push(await generator.nextId()));
        for (let i = 0; i < 99; i++) {
    ...
    it('Uniqueness', async () => {
        ...
        expect(values.filter((value, index, self) => self.indexOf(value) === index).length).toStrictEqual(samples);
    });
    ...
    it('Fast uniqueness', async () => {
        ...
        const samples = [];
        for (let i = 0; i < 10000; i++, samples.push(await flakeId.nextId()));
        const uniques = samples.filter((value, index, self) => self.indexOf(value) === index);
